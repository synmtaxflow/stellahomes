...
class LandingPageController extends Controller
{
    ...
    public function index()
    {
        ...
        $defaults = [
            'hero_image' => url('landing pages/img/hero.jpg'),
            'about_image' => url('landing pages/img/about.jpg'),
        ...
        foreach ($defaults as $key => $defaultValue) {
            ...
            if ($key === 'hero_image' || $key === 'about_image') {
                if (isset($rawSettings[$key]) && !empty($rawSettings[$key])) {
                    $settings[$key] = url('storage/' . $rawSettings[$key]);
                } else {
    ...
    public function book(Request $request)
    {
        ...
        try {
            ...
            if ($timeoutHours < 1 || $timeoutHours > 168) {
                $timeoutHours = 24; // Default to 24 hours if invalid
            }
            $expiresAt = Carbon::now()->addHours($timeoutHours);
            ...
            // Get owner account details for payment info
            $owner = User::where('role', 'owner')->first();
            $ownerDetail = $owner ? $owner->ownerDetail : null;
            $accountInfo = '';
            if ($ownerDetail && $ownerDetail->account_number) {
                $accountInfo = "Lipa kwenye Akaunti: {$ownerDetail->account_number} ({$ownerDetail->bank_name}) - {$ownerDetail->account_name}";
            }
            ...
            // Send SMS to student with control number and account info
            $studentMessage = "Karibu {$hostelName}! Booking yako imekubaliwa. Control No: {$controlNumber}. Username: {$username}, Password: {$password}. {$accountInfo} Tafadhali lipa ndani ya masaa {$timeoutHours} ili kuhifadhi booking yako. Asante!";
            ...
            if ($owner && $ownerDetail && $ownerDetail->phone_number) {
                $ownerMessage = "Booking mpya! Control No: {$controlNumber}. Mwanafunzi: {$request->full_name}, Namba: {$studentNumber}, Simu: {$request->phone}, Chumba: {$room->name}";
                if ($room->has_beds && $bed) {
                    $ownerMessage .= ", Kitanda: {$bed->name}";
                }
            ...
            if ($request->ajax()) {
                return response()->json([
                    'success' => true,
                    'message' => 'Booking successful! Your login credentials have been sent to your phone.',
                    'booking_info' => [
                        'username' => $username,
                        'password' => $password,
