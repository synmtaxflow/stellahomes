    ...
    public function index()
    {
        ...
        foreach (['hero_image', 'about_image'] as $imageKey) {
            if (isset($rawSettings[$imageKey]) && $rawSettings[$imageKey]) {
                ...
                $settings[$imageKey] = url('storage/' . $rawSettings[$imageKey]);
            } else {
        ...
        if ($hostelDetail->logo) {
            $settings['hostel_logo'] = url('storage/' . $hostelDetail->logo);
    ...
    public function updateContact(Request $request)
    {
        try {
            $request->validate([
                ...
                'booking_timeout_hours' => 'required|integer|min:1|max:168',
            ]);
        } catch (\Illuminate\Validation\ValidationException $e) {
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json([
        ...
        try {
            ...
            $contactData = [
                ...
                'booking_timeout_hours' => (int) $request->input('booking_timeout_hours'),
            ];
            
            // Validate booking timeout
            if ($contactData['booking_timeout_hours'] < 1 || $contactData['booking_timeout_hours'] > 168) {
                $contactData['booking_timeout_hours'] = 24;
            }
